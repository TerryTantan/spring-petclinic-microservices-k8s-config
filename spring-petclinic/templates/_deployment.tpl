    ...
    spec:
      containers:
        - name: {{ .root.containerName }}
          image: {{ and (not (empty .image.repository)) (not (contains "/" .root.image.name)) | ternary (print $.image.repository "/") "" }}{{ .root.image.name }}:{{ required "A tag must be provided" .image.tag }}
          ports:
